...
├── M_Phase                  │
├── M_Domain                 ←┘
├── IDX_DocumentBase         ← 全 Collection の一覧（Index）
└── <Collection シート群>         ← DOC-TECH-01, DOC-ENV-01, ...
```
...
Collection は書籍に相当する概念であり、ヘッダー情報テーブル（書誌情報）+ ドキュメントテーブル（目次・章一覧）で構成する。
...
│  │ collection_name      │ Gitの使い方                       │  │
│  │ summary          │ Git運用に必要な知識をまとめた…      │  │
│  │ domain           │ Technology                       │  │
...
|---|--------|------|-------------|
| 1 | no | number | 主キー。Collection 内通番。 |
...
| # | column | type | description |
|---|--------|------|-------------|
| 16 | document_id | string | 導出値: Collection ID + doc_type接頭辞 + 種別内通番 → `DOC-TECH-01-M01` |
...
全 Collection を横断した一覧シート。Collection シートの Tbl:HeaderInfo から自動集約する。
...
| `IDX_DocumentBase` | `Tbl:IDX_CollectionList` | Collection 横断一覧 |
...
| 出力先導出 | 個別登録制 | Collection ID から自動導出 |
...
| mode | 動作 |
|------|------|
| `auto` | output_root + collection_id で自動出力（デフォルト） |
...
| 新Collection 追加時 | パス登録が必要 | 登録不要（自動導出） |
