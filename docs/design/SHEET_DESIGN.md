...
Series は書籍に相当する概念であり、ヘッダー情報テーブル（書誌情報）+ ドキュメントテーブル（目次・章一覧）で構成する。
...
Series の書誌情報。書籍の表紙・奥付に相当する。

| key | type | description |
|-----|------|-------------|
| series_id | string | Series ID（シート名と同一） |
| series_name | string | Series の名称（書籍名に相当） |
| summary | string | Series の概要・目的 |
| domain | select | 主題領域（M_Domain） |
| related_project | string | FlowBase プロジェクトへの参照（任意） |
| status | select | Series のステータス（active / done / archived） |
| created | date | 作成日 |
| updated | date | 最終更新日 |
