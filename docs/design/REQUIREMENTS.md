...
| 用語 | 定義 | 書籍の類比 | FlowBase 対応 |
|------|------|------------|---------------|
| **Series** | 同一の主題（トピック）に属するドキュメント群。1シート = 1 Series。書籍に相当する概念。doc_type 混成を許容し、トピック単位で束ねる。アーカイブズ学（ISO 15489 / ISAD(G)）における標準術語。 | 書籍 | Project |
| **Document** | Series 内の個別ドキュメント。テーブルの1行に対応。基本数ページの単位。 | 章・ページ | Task |
| **Section** | Document の子要素（章・節）。当面は Obsidian 側で管理。 | 節・段落 | — |
| **M_\* シート** | プロパティの選択肢を定義するマスタシート。ヘッダー領域にメタ情報を持つ。 | — | — |
| **DEF_\* シート** | M_\* シートから自動生成される定義・集約ビュー。 | — | — |

### Series とプロジェクトの関係

プロジェクト（FlowBase）は Series を産むが、Series はプロジェクトから独立して存在する。
...
Series ヘッダーには `related_project` として FlowBase プロジェクトへの参照を任意で持てるが、必須ではない。
